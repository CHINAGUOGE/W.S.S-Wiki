...
- 先不急于给体验定性，先观察 **信息是否带来新内容**、**是否有稳定重复模式**、**是否能帮助协作**。
- 若每次都只能得到模糊回声，可先从外部媒介开始，例如便签、打字、录音，而不是强求内在对话必须立刻清晰。

### 2. “一到关键时刻就失联”

- 这通常意味着系统在高压下掉出了[可耐受窗口](Window-of-Tolerance.md)，优先级应回到稳定，而不是继续追问。
- 可预先准备最小交接清单，让前台成员在失联前至少留下一条信息。

### 3. “沟通一展开就吵起来”

- 把“讨论问题”和“处理情绪”拆开。先用[接地（Grounding）](Grounding.md)或暂停机制让系统回稳，再回到议题。
- 对高冲突话题设置时长上限，避免在疲劳或深夜做重要协商。
...
- 若系统经常出现时间缺失，建议把本页与[失忆（Amnesia）](Amnesia.md)、[切换（Switch）](Switch.md)一起阅读。
- 若某些成员只能以很弱的体感出现，可结合[存在感（Sense of Presence）](Sense-Of-Presence.md)建立更细的识别线索。
- 若内部讨论一碰创伤内容就迅速失稳，可先改为记录触发、身体感受与风险等级，暂缓深入叙事。
