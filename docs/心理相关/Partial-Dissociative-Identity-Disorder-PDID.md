---
comments: true
description: PDID（部分解离性身份障碍）是 ICD-11 中与 DID 相邻的解离障碍诊断，强调非主导身份的侵入体验、较少完全切换与较轻的解离性遗忘。本文帮助快速区分 PDID、DID 与 OSDD。
synonyms:
...
在 DSM-5 / DSM-5-TR 中，并未单列“部分解离性身份障碍（PDID）”这一正式诊断。
在临床实践中，症状未完全满足解离性身份障碍（DID）标准、但以身份分化为核心表现的个体，通常被归入 OSDD（Other Specified Dissociative Disorder）。
在临床教学与研究语境中，部分文献和培训材料将此类表现非正式地细分为：

- OSDD-1a（非官方术语）：

  具备与 DID 相似的身份分化结构，但身份间的切换不明显，或遗忘程度不足以满足完整 DID 标准。

- OSDD-1b（非官方术语）：

  存在较为清晰的身份分化与身份中断体验，但缺乏充分证据表明不同身份轮流控制行为，或其控制呈持续重叠状态。
因此，在跨诊断系统沟通中，ICD-11 所描述的部分解离性身份障碍（PDID）在临床表型上，常被认为与 DSM-5 体系中的 OSDD（尤其是 OSDD-1 描述性亚型）具有高度重叠性，但二者并非严格的一一对应诊断。
实务中通常通过具体症状表现、身份结构特点及功能受损程度进行对照描述。
---
