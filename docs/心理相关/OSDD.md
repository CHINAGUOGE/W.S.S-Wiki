...
??? question "OSDD 是什么？"
    OSDD 是 **Other Specified Dissociative Disorder** 的缩写，中文常译为 **其他特定解离性障碍**。它用于描述那些存在显著解离症状、也有现实痛苦或功能影响，但又 **没有完全符合 DID 或其他单一解离障碍完整标准** 的情况。

??? question "OSDD 和 DID 的区别是什么？"
    可以把 OSDD 理解为与 DID 很接近、但不完全重合的一组临床表现。DID 更强调明确的身份状态切换与反复的解离性遗忘；OSDD 则常见“有身份差异但失忆不够典型”或“有明显解离症状但不满足完整条目”的情况。两者都需要正式评估，不能只凭社群经验词判断。

??? question "OSDD-1a、OSDD-1b 是正式诊断吗？"
    不是。`OSDD-1a`、`OSDD-1b` 是互助社群里常见的经验标签，用来粗略描述“身份差异更明显”或“遗忘更明显”等模式，但 **DSM-5-TR 和 ICD-11 都没有把它们列为正式亚型**。临床文书通常只写 OSDD，并说明不完全符合标准的具体原因。

??? question "OSDD 算不算“多重人格”？"
    大众有时会把 OSDD 和 DID 一起归进“多重人格”这类俗称，但这种说法不够准确。更合适的做法是直接使用 **OSDD** 或 **DID** 这样的规范名称，并区分是否存在身份切换、记忆缺口、功能损害与创伤相关背景。

??? question "OSDD 和 PDID、DID 的区别怎么看最快？"
    如果你只想先快速区分：DID 更强调 **轮流接管 + 明显失忆**；PDID 更强调 **侵入多于完整接管**；OSDD 则是 **症状已明显但边界还不完全落入 DID 或其他单一诊断**。更完整的速查表可看 [DID、OSDD、PDID 有什么区别？](DID-OSDD-PDID-Comparison.md)。
