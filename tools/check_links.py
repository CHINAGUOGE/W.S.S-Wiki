...
3. 其他目录到词条：使用 `../../心理相关/` 路径（如 `../../心理相关/DID.md`）
4. 禁止：绝对路径（如 `/docs/../心理相关/DID.md`）
...
def validate_link(
    ...
    if is_absolute_path(pure_target):
        return False, f"禁止使用绝对路径，请使用相对路径（如 `../../心理相关/xxx.md`）"
    ...
    if source_context == "entries":
        # 词条文件 (docs/../心理相关/xxx.md)

        if target_in_entries:
            # 链接到其他词条：应该直接使用文件名
            # 正确：DID.md
            # 错误：../../心理相关/DID.md, docs/../心理相关/DID.md
            ...
            if not pure_target.startswith("../"):
                return False, (
                    f"词条链接到其他目录应使用 `../` 相对路径\n"
                    f"    当前：{pure_target}\n"
                    f"    提示：从 docs/../心理相关/ 到目标需要先 ../ 回到 docs/"
                )
            return True, ""
        ...
        if target_in_entries:
            # 链接到词条：应该使用 ../../心理相关/ 路径
            if pure_target.startswith("../../心理相关/"):
                return True, ""
            else:
                return False, (
                    f"从 docs/ 子目录链接到词条应使用 `../../心理相关/` 路径\n"
                    f"    当前：{pure_target}\n"
                    f"    应改为：../../心理相关/{target_path.name}"
                )
        else:
            # 链接到非词条文件：相对路径即可
            return True, ""
        ...
        if target_in_entries:
            # 链接到词条：应该使用 ../心理相关/ 或 docs/../心理相关/ 格式
            # 从 docs/ 根目录：../心理相关/DID.md
            # 从仓库根目录：docs/../心理相关/DID.md
            ...
            if source_context == "docs_root":
                ...
                if pure_target.startswith("../心理相关/"):
                    ...
                    return False, (
                        f"从 docs/ 根目录链接到词条应使用 `../心理相关/` 路径\n"
                        f"    当前：{pure_target}\n"
                        f"    应改为：../心理相关/{target_path.name}"
                    )
            else:
                # 从仓库根目录
                if pure_target.startswith("docs/../心理相关/"):
                    return True, ""
                else:
                    return False, (
                        f"从仓库根目录链接到词条应使用 `docs/../心理相关/` 路径\n"
                        f"    当前：{pure_target}\n"
                        f"    应改为：docs/../心理相关/{target_path.name}"
                    )
        else:
            # 链接到非词条文件：相对路径即可
            return True, ""
...
用法示例：
  ...
  python3 tools/check_links.py docs/../心理相关/      # 检查词条目录
  python3 tools/check_links.py docs/../心理相关/DID.md  # 检查单个文件
  python3 tools/check_links.py --root /path/to/repo docs/../心理相关/  # 指定仓库根目录
        """
    )
    parser.add_argument(
        "path",
