...
TOPIC_INDEX_DESCRIPTIONS = {
    "诊断与临床": "聚合“诊断与临床”主题下的全部词条，覆盖解离障碍、创伤相关障碍、人格障碍与常见鉴别诊断，便于按目录快速浏览。",
    "系统运作": "聚合“系统运作”主题下的全部词条，涵盖切换、共意识、内部沟通、记忆管理与系统治理等核心机制。",
    "实践指南": "聚合“实践指南”主题下的全部词条，覆盖 Tulpa 创造、意识训练、危机应对与内部协作技巧，便于按主题快速查找。",
    "创伤与疗愈": "聚合“创伤与疗愈”主题下的全部词条，覆盖稳定化、自我调节、创伤治疗模型与常见疗法，便于按主题浏览。",
    "角色与身份": "聚合“角色与身份”主题下的全部词条，涵盖宿主、保护者、守门人、照护者等常见系统角色与身份分类。",
    "理论与分类": "聚合“理论与分类”主题下的全部词条，覆盖结构性解离、依恋理论、自我发展、防御机制与心理学流派等框架。",
    "文化与表现": "聚合“文化与表现”主题下的全部词条，覆盖影视、文学、动画与游戏中的多意识体和解离相关呈现。",
}
        ...
        if use_letter_grouping:
            ...
            for idx, letter in enumerate(sorted_letters):
                ...
                for item in group_items:
                    lines.append(f"        * [{item['title']}](../心理相关/{item['filename']})")
        else:
            # 不分组,直接按标题排序
...
def main():
    ...
    for topic, filename in TOPICS.items():
        ...
        lines.append(f"description: {TOPIC_INDEX_DESCRIPTIONS[topic]}")
        ...
        lines.append("")
        lines.append(f"> 本页汇总所有主题为「{topic}」的词条，原文档仍在 `../心理相关/` 目录中。")
        lines.append("")
        ...
        if items:
            ...
            for it in items:
                ...
                lines.append(f"- [{it['title']}](../../心理相关/{it['filename']}){suffix}")
        else:
            lines.append("*暂无词条*")
